...
export default function QuizPlayer() {
  ...
  return (
    <div className="container mt-2 mt-md-4 px-3 px-md-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-2">
        <div className="small small-md">Question {index + 1}/{total}</div>
        <div className="fw-bold small small-md">{Math.floor(timeLeft / 60)}:{String(timeLeft % 60).padStart(2, '0')}</div>
      </div>
      <ProgressBar now={progress} className="mb-3" />
      <div className="card p-3 p-md-4 quiz-player-card">
        <h5 className="mb-3 mb-md-4 quiz-question-text">{q.text}</h5>
        <div className="mt-3 quiz-options">
          {q.options.map((opt, i) => {
            const isSelected = answers.find(a => a.index === index && a.chosenIndex === i);
            return (
              <div
                key={i}
                className={`option p-3 p-md-3 mb-2 mb-md-3 border rounded ${isSelected ? 'border-primary bg-light' : 'border-light'}`}
        ...
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-center gap-2 gap-md-3 mt-4 quiz-buttons">
          <Button 
            variant="secondary" 
            disabled={index === 0} 
            onClick={() => { setIndex(i => i - 1); setTimeLeft(quiz.perQuestionSeconds || 60); }}
            className="quiz-btn quiz-btn-previous"
          >
            Previous
          </Button>
          <div className="d-flex gap-2 quiz-btn-group">
            <Button 
              variant="danger" 
              className="quiz-btn quiz-btn-quit"
              onClick={() => { exitFullscreen(); navigate('/student'); }}
            >
              Quit
            </Button>
            <Button 
